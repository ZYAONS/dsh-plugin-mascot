...
- **三个角色、七套官方形象，随时切换** —— 可露希尔与缪尔赛思（明日方舟）× 千石由乃（梦限大 MewType）
...
可露希尔与缪尔赛思 © 鹰角网络《明日方舟》，千石由乃 © Bushiroad《BanG Dream!》/ 梦限大 MewType ——
...
- **02 角色** —— 可露希尔（罗德岛工程终端）／千石由乃（MEWTYPE LIVE）／缪尔赛思（莱茵生命生态终端）
- **03 形象** —— 勾选允许出现的立绘；勾掉的会写进 allowlist，插件连列都不列
- **04 动效** —— 骨骼动效、多帧形象
- **05 输出** —— 生成可直接粘贴的 `cordis.patch.yml`，一键复制或下载
- **06 安装** —— 五步走完，含素材下载与重启

### 四套网页主题：每位角色一套，外加一套中性

配置台为每位角色准备了一套界面风格，而且它们不是换个颜色就算数：

| | 中性（`auto`） | 可露希尔 | 千石由乃 | 缪尔赛思 |
|---|---|---|---|---|
| 底色 | 中性炭黑 | 蓝黑 | 紫黑 | 暗绿 |
| 信号色 | 银 | 蓝 | 洋红 | 浅绿 |
| 直角 / 圆角 | 直角 | 直角 | 14px 圆角 | 10px 圆角 |
| 选中提示 | 四角括号 | 四角括号 | 辉光 | 内描边 |
| 标签字体 | 等宽 | 等宽 | 圆体 | 无衬线 |

**选了某位角色的主题，就等于选了那位角色。** 点由乃的主题，画面就切成由乃；
点由乃本人，主题也切成她的 —— 无论从哪个控件操作，两者永远一起走，
因为「用着 A 的配色却显示 B」就是页面在自相矛盾。
中性主题是那条规则的反证：它不属于任何角色，所以可以放心选，不会动到画面上的人，
也是唯一允许和立绘不一致的一套。

两套都支持深链，`?theme=` 会连着角色一起切：
...
缪尔赛思的小人来自同一个仓库，同样会绑骨；moegirl 上的那几套（两位立绘和由乃的三套）不发这个头，
...
不是拍脑袋定的。明日方舟的基建小人是 **Spine 3.8 模型**，而且读得出来 ——
[Ark-Models](https://github.com/isHarryh/Ark-Models) 有镜像，官方 3.8 运行时能解析：

```bash
npm run rig:reference            # 可露希尔基建小人，Relax 动作
```
...
关键的是最后一列。这套骨骼原来点头 **±2.4°、周期八秒** —— 幅度大了好几倍、慢了四倍，
所以看起来是「慢慢歪过去」而不是呼吸。现在跑在实测出来的 **2.00 秒**节拍上。

角度**不是**照抄的：游戏里是 297 根骨头，这里是 3 根，同一个角度在两边不是一回事。
周期是，数量级是。

运行时不进仓库、模型也不进仓库 —— 都落在被忽略的 `.cache/spine` 里，
因为运行时是 Spine 的（Spine Runtimes License），模型是鹰角的。
脚本只抓那一个 `.skel`，不去碰整个约 1 GB 的仓库，也完全跳过贴图集：
它给读取器真实的 attachment 类加一个占位贴图区域，不做图也能把账算平。
...
立绘下面那颗胶囊显示三个数字，**能测到哪个显示哪个**：

| | 从哪来 |
| --- | --- |
| **余额** `¥128.42` | 主机侧向 DeepSeek 查，API Key 不进浏览器 |
| **命中** `89.0%` | 本会话的缓存命中率（缓存读取 / 计费输入） |
| **今天** `1.2M` | 本机当天累计消耗的 token |

查询失败时显示「点我 Token / 余额」，不会假装自己知道。

**「今天」为什么需要主机侧记账。** 浏览器知道**本会话**花了多少 —— 会话投影里就有；
但没有任何东西知道**这一天**花了多少：一小时前结束的会话把数字带走了，下一个会话从零开始。
所以主机侧留了一份当日流水（`.cache/usage-daily.json`，gitignore）。

记账规则是**每会话取高水位、差值累加到当天**。浏览器上报的是会话累计值，而且会反复上报，
所以重复上报必须加零 —— 否则面板每两秒轮询一次，这一天就会按轮询频率膨胀，而那个数字
看起来还像是个测量结果。会话总数倒退时（id 复用或投影重置）不倒扣，只保住高水位。
...
每个角色身后都有一间房。**仓库里没有任何一张背景图** —— 它要么是插件用渐变画出来的，
要么是你自己下载到本机的官方家具素材。

插件对每个角色都画一间（可露希尔是她的工作间：深蓝房间、L 形白桌、两块屏幕、桌下青色
灯带、墙上海报、地板上的触手纹地毯）。想换成官方家具图就下载：

```
npm run fetch-rooms          # 下载缺的
npm run fetch-rooms -- --list  # 只看清单，不下载
npm run fetch-rooms -- --force # 重下
```

素材放在 `art/room-<角色 id>.<png|jpg|webp>`，宿主按文件名找它们（`readRooms`），
所以下完重启即可，不需要重新生成索引。清单在 `art/rooms.json`，逐个角色写了用的是哪套家具、
是不是本人套装、以及来源页面。

**这些是鹰角网络的官方素材，不要提交进仓库。** `art/` 已被 gitignore，`npm test` 也会在
任何图片被 git 跟踪时失败 —— 包括背景图。删掉 `art/room-*` 就退回画出来的那间房。
