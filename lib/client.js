window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const THEMES = {
			...
			// 缪尔赛思 — a Rhine Lab terminal: clean, scientific, water. Thin rules,
			// a hairline meter and ripple rings, and a plain sans rather than the
			// console's monospace or the stage's rounded face.
			rhine: {
				label: "莱茵生命 · 生态终端",
				accent: "#5fb8f0",
				accentSoft: "rgba(95, 184, 240, 0.14)",
				accentLine: "rgba(95, 184, 240, 0.55)",
				surface: "linear-gradient(158deg, rgba(18,30,40,.97), rgba(8,14,20,.99))",
				hairline: "rgba(140, 200, 240, 0.16)",
				text: "#e6f0f8",
				dim: "rgba(230, 240, 248, 0.5)",
				radius: "10px",
				radiusSm: "6px",
				shape: "lab",
				bar: "hairline",
		...
		function poseRig(rig, box, state) {
			...
			// Amplitudes are small on purpose: this is an idle, not a dance. Large
			// angles on a two-dimensional image read as rubber, not as breathing.
			const sway = Math.sin(t * 0.62) * 1.5 * rad;
			const spineAngle = Math.sin(t * 0.9 + 0.7) * 1.1 * rad;
			const nod = Math.sin(t * 0.78 + 1.4) * 2.4 * rad + Math.sin(t * 2.1) * 0.5 * rad;
			const breathe = 1 + Math.sin(t * 1.25) * 0.011;
			const bounce = Math.sin(t * 1.25 + Math.PI / 2) * box[3] * 0.006;
				...
				image.onload = () => {
					...
					const loop = (now) => {
						...
						if (pokeRef.current !== undefined) {
							pokeRef.current += (now - (last ?? now)) / 1000;
						...
						last = now;
						skinner.draw(poseRig(rig, box, { time: elapsed, pokeAge: age }));
					};
					raf = window.requestAnimationFrame(loop);
