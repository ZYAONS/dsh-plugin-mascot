...
.dsh-mascot-panel { position: absolute; right: 0; bottom: calc(100% + 12px); width: 344px;
  ...
  /* The panel is anchored to the dock, which sits 20px off the bottom with a 172px sprite and a
     chip below the panel — so its ceiling has to leave room for all of that plus the panel's own
     12px gap and a margin. At 100vh minus 248px a full panel's top landed above the frame and the
     heading went off screen; the music row is what finally made it visible. */
  max-height: calc(100vh - 320px); overflow-x: hidden; overflow-y: auto;
...
`;
		 ...
		 */
		function SkinnedSprite({ look, className, seat, seatSize, onUnavailable, gesture }) {
		...
		function Sprite({ look, className, seat, seatSize, useRig, onUnavailable, gesture }) {
			...
			if (rigged) {
				return h(SkinnedSprite, { look, className, seat, seatSize, onUnavailable, gesture });
			}
			if (frames.length === 1) {
					...
					h(
						...
						h(
							...
							h("span", {
								...
								style: roomFile === undefined ? undefined : { backgroundImage: `url("${roomUrl(roomFile)}")` },
							}),
							h("span", { className: "dsh-mascot-decor" }),
							h("span", { className: "dsh-mascot-frames", ref: spriteRef }, h(Sprite, { look, className: "dsh-mascot-sprite", seat: "sprite", seatSize: SPRITE_SEAT, useRig: rigOn && !rigFailed, gesture: character?.gesture, onUnavailable: () => setRigFailed(true) })),
