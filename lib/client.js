...
.dsh-mascot-root[data-shape="console"] .dsh-mascot-scene {
  background:
    linear-gradient(to top, #171d27 0 21%, transparent 21%),
    repeating-linear-gradient(90deg, rgba(255,255,255,.045) 0 1px, transparent 1px 26px),
    linear-gradient(168deg, #202a38, #131922 70%); }
.dsh-mascot-root[data-shape="console"] .dsh-mascot-scene::before {
  left: 10px; right: 10px; top: 30px; height: 3px; background: rgba(120, 200, 210, .5);
  box-shadow: 0 0 0 1px rgba(0,0,0,.35), 0 9px 0 -1px rgba(255,255,255,.06),
    14px -11px 0 -3px rgba(255,255,255,.1), 30px -9px 0 -4px rgba(255,255,255,.08),
    46px -13px 0 -3px rgba(255,255,255,.12); }
.dsh-mascot-root[data-shape="console"] .dsh-mascot-scene::after {
  left: 50%; top: 0; width: 34px; height: 20px; margin-left: -17px;
  background: radial-gradient(ellipse at 50% 0%, rgba(140, 230, 225, .5), transparent 70%);
  border-top: 2px solid rgba(160, 240, 235, .55); border-radius: 0 0 16px 16px; }
		...
		function SkinnedSprite({ look, className, seat, seatSize, onUnavailable }) {
			...
			React.useEffect(() => {
				...
				image.onload = () => {
					...
					const rig = buildRig(frame.profile, eyes === null ? BODY_MESH : EYE_MESH);
					...
					const loop = (now) => {
						...
						const close = sampleBlink(elapsed, greetAge);
						jellyRef.current = stepJelly(jellyRef.current, close, step);
						skinner.draw(
							poseRig(rig, box, { time: elapsed, pokeAge: age, greetAge }),
				...
				h(
					...
					h(
						...
						h(
							...
							h("span", { className: "dsh-mascot-scene" }),
