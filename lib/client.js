	...
	factory: (require) => {
		...
		const THEMES = {
			...
			miuyin: {
				label: "明日方舟 · 天蓝",
				accent: "#3fa9f5",
				accentSoft: "rgba(63, 169, 245, 0.15)",
				accentLine: "rgba(63, 169, 245, 0.58)",
				surface: "linear-gradient(158deg, rgba(16,30,44,.97), rgba(7,13,20,.99))",
				hairline: "rgba(120, 190, 245, 0.16)",
				text: "#e4f0fb",
...
/**
 * What the official recording actually says, where the text is known.
 *
 * This exists because the lines above are written for this project and the recordings are
 * the game's: showing one while playing the other is a mismatch the ear catches
 * immediately. When a file is played, the subtitle has to be that file's line.
 *
 * `label` is the line's name in the game — 「戳一下」 is the tap response. Where the wording
 * has not been transcribed, only the label is shown: a missing sentence is honest, a wrong
 * sentence is not.
 */
const VOICE_FILE_LINES = {
  // The text is the game's own, taken from the same voice entry the downloaded recording
  // belongs to (voice index 34, "戳一下" — the tap response). The Chinese line is what
  // the Japanese recording says, so the two finally agree.
...
function speakLine(characterId, options = {}) {
  ...
  if (line === undefined) {
    ...
    return { ja: "", zh: "", label: "", spoke: false, source: null, reason: lastVoiceError };
  }
  ...
  if (typeof file === "string" && file !== "") {
    try {
      ...
      lastVoiceError = null;
      // The official text, not the written one: the recording is the game's, so the
      // subtitle has to be the same sentence the voice is saying.
      ...
      return { ja: line.ja, zh: line.zh, label: "", spoke: false, source: "file", reason: lastVoiceError };
    }
  }
  if (typeof window === "undefined" || window.speechSynthesis === undefined) {
    lastVoiceError = "this browser has no speech synthesis";
    return { ja: line.ja, zh: line.zh, label: "", spoke: false, source: null, reason: lastVoiceError };
  }
  ...
  lastVoiceError = null;
  return { ja: line.ja, zh: line.zh, label: "", spoke: true, source: "synthesis", reason: null };
}
//#endregion
...
.dsh-mascot-panel { position: absolute; right: 0; bottom: calc(100% + 12px); width: 344px;
  max-width: calc(100vw - 40px); max-height: calc(100vh - 248px); overflow-x: hidden; overflow-y: auto;
...
`;
		 ...
		 */
		function SkinnedSprite({ look, className, seat, seatSize, onUnavailable }) {
			...
			React.useEffect(() => {
				...
				let age;
				let greetAge;
				/** Moves the eyelids, or undefined when this look has no eye boxes. */
				let blink;
				const image = new Image();
				image.decoding = "async";
				image.onload = () => {
					if (cancelled || host.current === null) return;
					const rig = buildRig(frame.profile, { rows: 26, cols: 18 });
					const built = createSkinner(image, rig, framing, box, seatSize);
					if (built === undefined) {
						unavailable.current?.();
						return;
					}
					...
					// Blink on the schedule the game uses. The boxes come from the index:
					// measured from the artwork where that worked, from the official rig's
					// own eyelash bones otherwise.
					blink = createBlink(host.current, {
						width: frame.measured?.width ?? 0,
						height: frame.measured?.height ?? 0,
						body: frame.body ?? null,
						eyes: frame.eyes ?? null,
						colour: Array.isArray(frame.skin) ? `rgb(${frame.skin.join(",")})` : undefined,
					});
					...
					const loop = (now) => {
						...
						if (greetRef.current !== undefined) {
							...
							if (greetRef.current > SPINE_MOTION.greet.loop) greetRef.current = undefined;
							greetAge = greetRef.current;
						} else {
							greetAge = undefined;
						}
						last = now;
						skinner.draw(poseRig(rig, box, { time: elapsed, pokeAge: age, greetAge }));
						blink?.(elapsed);
					};
					raf = window.requestAnimationFrame(loop);
		 ...
		 */
		function Sprite({ look, className, seat, seatSize, useRig, onUnavailable }) {
			...
			const rigged = useRig === true && seatSize !== undefined && frames[0]?.profile != null;
			const animated = frames.length > 1 && !rigged;
			React.useEffect(() => {
			...
			if (rigged) {
				return h(SkinnedSprite, { look, className, seat, seatSize, onUnavailable });
			}
			if (frames.length === 1) {
		...
		function MascotPanel(props) {
			const {
				...
				rigOn, rigFailed, onToggleRig, showBalance,
			} = props;
				...
				h(
					...
					h(
						...
						h(
							...
							h("span", { className: "dsh-mascot-frames", ref: spriteRef }, h(Sprite, { look, className: "dsh-mascot-sprite", seat: "sprite", seatSize: SPRITE_SEAT, useRig: rigOn && !rigFailed, onUnavailable: () => setRigFailed(true) })),
