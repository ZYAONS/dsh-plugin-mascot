window.__ModuleLoader__.load({
  ...
  <circle cx="124" cy="202" r="5" fill="#ff5f9e"/>
		 ...
		 *
		 * `art` names the official artwork served by the host half; `face` frames
		 * that same file into the small portrait the panel header shows, in the
		 * wrapper's own pixels (`width` of the image plus the offsets that put the
		 * head inside a 38x50 window). `svg` is the built-in vector fallback used
		 * whenever `art/` is empty — the plugin must never render a blank sprite.
		 */
		const MASCOTS = [
			{
				id: "closure",
				...
				art: "closure.png",
				// The official 立绘 is 1024x1024 with the figure centred and roughly 78%
				// transparent margin, so the sprite seat renders it oversized and pulls
				// it into place. `face` frames the head into the 38x50 panel portrait.
				sprite: { width: 208, left: -51, top: -14 },
				face: { width: 116, left: -40, top: -4 },
				svg: CLOSURE_SVG,
			},
			{
				id: "yuno",
				...
				art: "yuno.png",
				// The official 立绘 is 1550x2085 (aspect 0.74); the figure already fills
				// the canvas, so the sprite seat needs far less oversizing.
				sprite: { width: 146, left: -20, top: -4 },
				face: { width: 116, left: -47, top: -6 },
				svg: YUNO_SVG,
			},
		];
...
`;
		...
		function Art({ mascot, className, frame }) {
			...
			if (url === undefined || failed) {
				return h("span", { className: `dsh-mascot-art ${className}`, dangerouslySetInnerHTML: { __html: mascot.svg } });
			}
		...
		function Face({ mascot }) {
			...
			if (url === undefined || failed || mascot.face === undefined) {
				return h("span", { className: "dsh-mascot-art dsh-mascot-face", dangerouslySetInnerHTML: { __html: mascot.svg } });
			}
			return h(
				"span",
