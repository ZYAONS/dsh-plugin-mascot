...
`;
		...
		/** The remembered bone-rig preference, defaulting to on. */
		function readStoredRig() {
			try {
				return window.localStorage.getItem(RIG_KEY) !== "off";
			} catch {
				return true;
			}
		}
		...
		function MascotOverlay(props) {
			...
			const [rigOn, setRigOn] = React.useState(readStoredRig);
