...
const DEFAULTS = Object.freeze({
	...
	balance: true,
	/**
	 * Origins allowed to read the artwork route without a browser session.
	 *
	 * The web console previews the art you installed, and it is served from a
	 * different origin, so it cannot carry the DSH session cookie. Rather than
	 * weaken the route in general, this is an exact-match allowlist that applies to
	 * `/art/` only — balance and look-index reads still require the cookie. Set it
	 * to an empty list to switch the cross-origin preview off entirely.
	 */
...
function normalizeConfig(raw) {
	...
	return {
		...
		artOrigins: Array.isArray(source.artOrigins)
			? source.artOrigins.filter((entry) => typeof entry === "string" && entry.length > 0)
			: DEFAULTS.artOrigins,
	};
}
 ...
 * @param allowOrigin - an allowlisted Origin to answer with, when the caller is a
 *   cross-origin reader like the web console.
 */
function sendJson(res, status, payload, allowOrigin) {
	const body = JSON.stringify(payload);
	res.writeHead(status, {
		"content-type": "application/json; charset=utf-8",
		"content-length": Buffer.byteLength(body),
		"cache-control": "no-store",
		...(allowOrigin === undefined ? {} : { "access-control-allow-origin": allowOrigin, vary: "origin" }),
	});
	res.end(body);
}
 ...
 * @param allowOrigin - the request Origin when it is on the artwork allowlist.
 */
async function serveArt(res, config, name, allowOrigin) {
	/** Headers every art answer carries; CORS only when an allowed origin asked. */
	const headers = (extra) => ({
	...
	if (target !== config.artDir && !target.startsWith(config.artDir + sep)) {
		sendJson(res, 403, { ok: false, error: "forbidden", message: "素材路径越界" }, allowOrigin);
		return;
	}
	const mime = ART_MIME[extname(name).toLowerCase()];
	if (mime === undefined) {
		sendJson(res, 404, { ok: false, error: "not_found", message: `不支持的素材类型：${name}` }, allowOrigin);
		return;
	}
	let body;
	try {
	...
	res.writeHead(200, headers({ "content-type": mime, "content-length": body.length }));
	res.end(body);
}

/** The request's Origin, when the operator has allowlisted it for the art route. */
function artOriginOf(req, config) {
	const origin = req.headers.origin;
	return typeof origin === "string" && config.artOrigins.includes(origin) ? origin : undefined;
}
...
function apply(ctx, rawConfig) {
	...
	async function handle(req, res) {
		...
		// The artwork route is the one seat a cross-origin reader may reach, and only
		// from an origin the operator allowlisted. Everything else — the balance,
		// the look index — still needs the DSH browser session.
		if (route.startsWith("/art/")) {
			const allowOrigin = artOriginOf(req, config);
			if (allowOrigin === undefined && !authorized(ctx, req)) {
				sendJson(res, 401, { ok: false, error: "unauthorized", message: "需要 DSH Web 会话认证" });
				return;
			}
			if (req.method === "HEAD") {
				res.writeHead(200, allowOrigin === undefined ? {} : { "access-control-allow-origin": allowOrigin, vary: "origin" });
				res.end();
				return;
			}
			await serveArt(res, config, route.slice("/art/".length), allowOrigin);
			return;
		}
