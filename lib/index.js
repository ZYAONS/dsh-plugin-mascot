...
/**
 * Read one of the two music declarations, by the key its entries live under.
 *
 * Both files are small and both are hotlink-only — a cid, a title, and a URL on somebody else's
 * CDN — so they are handed to the browser whole rather than filtered per character. An absent
 * or unreadable file is an empty map: a machine without the catalogue still gets a working
 * mascot, it just has nothing to play.
 *
 * @param config - resolved plugin config.
 * @param file - the declaration's filename under `art/`.
 * @param key - the property the entries live under inside it.
 * @returns character id -> music entry.
 */
async function readMusic(config, file, key) {
	try {
		const parsed = JSON.parse(await readFile(join(config.artDir, file), "utf8"));
...
/**
 * Which characters have a backdrop picture installed, and under what name.
 *
 * `art/room-<character id>.<ext>`. The plugin draws a room for every character already, from
 * gradients — this is the escape hatch for someone who wants the real thing for one of them.
 * It is deliberately read from `art/`, which is gitignored: official furniture and stage art
 * belongs to whoever drew it, and the repository ships the drawn version for that reason.
 * Nothing here is downloaded, and a missing file is the ordinary case.
 */
async function readRooms(config) {
	/** id -> `{ file, rank }`. The file keeps the spelling on disk. */
	const byId = new Map();
	let entries;
	try {
		entries = await readdir(config.artDir);
	} catch {
		return {};
	}
	for (const original of entries) {
		const match = /^room-([a-z0-9-]+)\.(png|jpe?g|webp)$/iu.exec(original);
		if (match === null) continue;
		const id = match[1].toLowerCase();
		const rank = ROOM_EXTENSIONS.indexOf(match[2].toLowerCase());
		if (rank < 0) continue;
		const best = byId.get(id);
		// First extension wins, so adding a png beside a webp does not silently change
		// which one is drawn.
		if (best === undefined || rank < best.rank) byId.set(id, { file: original, rank });
	}
	// The id is lower-cased for matching, the file is not: a lookup table that lower-cases
	// the name it reports sends the browser after a file that only exists on Windows.
	return Object.fromEntries([...byId].map(([id, hit]) => [id, hit.file]));
}

/** Image extensions a backdrop may use, in the order they are preferred. */
const ROOM_EXTENSIONS = ["webp", "png", "jpg", "jpeg"];
...
function apply(ctx, rawConfig) {
	...
	/**
	 * Where the daily total lives. Under `.cache/`, which is gitignored: it is this
	 * machine's runtime state, not part of the project.
	 */
	function usageLedgerPath() {
		return join(PLUGIN_ROOT, ".cache", "usage-daily.json");
	}

	/**
	 * Create or update the running total of Tokens spent today.
	 *
	 * The browser knows how much *this session* has spent, because the session projection
	 * says so. Nothing knows how much the day has spent: a session that ended an hour ago
	 * took its numbers with it, and the next one starts at zero. So the day is kept here,
	 * as a running total fed by whatever the browser last reported.
	 *
	 * The arithmetic is deliberately "high-water mark per session, difference added to the
	 * day". The figure the browser sends is cumulative for its session, so re-reporting the
	 * same number must add nothing — otherwise a panel polling every two seconds would
	 * inflate the day by its own polling rate, which is the sort of bug that reads as a
	 * real measurement.
	 *
	 * @param sessionId - which session the total belongs to; omit for a plain read.
	 * @param total - that session's cumulative spend; omit for a plain read.
	 * @returns `{ day, today }`.
	 */
	async function trackUsage(sessionId, total) {
		const now = new Date();
		// Local midnight, not UTC: "今天" is the day the person is having.
		const day = `${String(now.getFullYear())}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}`;
		let ledger = { day, total: 0, seen: {} };
		try {
			const parsed = JSON.parse(await readFile(usageLedgerPath(), "utf8"));
			// Yesterday's numbers are not today's. Carrying them over would make the first
			// session of the morning look like it had already spent a day's worth.
			if (parsed !== null && typeof parsed === "object" && parsed.day === day) {
				ledger = {
					day,
					total: Number(parsed.total) || 0,
					seen: parsed.seen !== null && typeof parsed.seen === "object" ? parsed.seen : {},
				};
			}
		} catch {
			// No ledger yet, or an unreadable one. Either way today starts at zero.
		}

		if (typeof total === "number" && Number.isFinite(total) && total >= 0 && typeof sessionId === "string" && sessionId !== "") {
			const previous = Number(ledger.seen[sessionId]) || 0;
			// A session's total only grows. A smaller one means a reused id or a projection
			// that reset, and either way the difference is not spend.
			if (total > previous) ledger.total += total - previous;
			ledger.seen[sessionId] = Math.max(previous, total);
			// Keep the file bounded: a long day would otherwise collect every session id.
			const ids = Object.keys(ledger.seen);
			for (const id of ids.slice(0, Math.max(0, ids.length - 64))) delete ledger.seen[id];
			try {
				await mkdir(dirname(usageLedgerPath()), { recursive: true });
				await writeFile(usageLedgerPath(), JSON.stringify(ledger), "utf8");
			} catch {
				// A ledger that cannot be written is a missing figure, not a broken panel.
			}
		}
		return { day: ledger.day, today: ledger.total };
	}

	/**
	 * Read a request body, with a cap.
	 *
	 * The body here is two small fields; anything larger is not this route's client.
	 * @param req - incoming request.
	...
	async function handle(req, res) {
		...
		// The ledger is the one thing the browser writes; everything else is a read. The
		// distinction is enforced per route rather than by opening POST up everywhere.
		if (route === "/api/usage") {
			if (method !== "POST") {
				const { day, today } = await trackUsage(config);
				sendJson(res, 200, { ok: true, day, today });
				return;
			}
			let body;
			try {
			...
			const { day, today } = await trackUsage(body.sessionId, body.total);
			sendJson(res, 200, { ok: true, day, today });
			return;
		}
		if (method === "POST") {
		...
		if (route === "/api/looks") {
			...
			sendJson(res, 200, {
				...
				// And which characters have a real backdrop picture installed. Absent for
				// everyone by default: the plugin draws a room per character instead, and
				// this is only for someone who wants the official art on their own machine.
				rooms: await readRooms(config),
				// Each character's music: the Siren Records album they belong to, and — for the
				// two characters Siren does not cover — a licensed preview of their theme song.
				// Only ids and titles live here; every cover and every audio file is hotlinked
				// from the rights holder's own CDN and never downloaded by this plugin.
				siren: await readMusic(config, "siren.json", "records"),
				songs: await readMusic(config, "songs.json", "songs"),
