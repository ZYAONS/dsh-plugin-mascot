...
打开 `GET /dsh-token-thrift/console/`。除了把上面的东西放大，它还能**改**：

- **切档位**：off / light / standard / strict 四个按钮，点了立刻生效 —— 不用重启，不用改 profile。
- **改预算**：填个数按应用。比率当场按新预算重算：教练自己的判断要等下一次工具调用，
  但"烧到哪了"是个事实，没理由显示旧的。
- **重置会话**：档位每会话只响一次，不重置就没法看它工作第二次。
- **原始 JSON**：折叠着，排查时展开。

写的都是**运行时覆盖**，重启回到 profile 里的值；页面同时显示"配置里是什么"和"现在是什么"，
以及哪几项被覆盖了。
...
41 项，**不启 DSH**：造一个够用的伪 ctx，把钩子拿出来直接驱动。测的是插件的逻辑（哪一档该响、
