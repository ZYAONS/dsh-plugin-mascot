...
await it("every character names a theme the stylesheet can actually draw", () => {
  ...
  for (const [id, theme] of Object.entries(THEMES)) {
    ...
    assert.ok(["console", "stage", "lab", "veil", "sky", "pool", "ink", "frame", "board"].includes(theme.shape), `${id}: unknown shape`);
    assert.ok(["segmented", "vu", "hairline", "beam", "blocks", "wave", "brush", "ticks", "grid"].includes(theme.bar), `${id}: unknown bar style`);
